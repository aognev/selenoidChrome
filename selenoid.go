...
func create(w http.ResponseWriter, r *http.Request) {
	...
	var userDirPath = os.TempDir() + "/.selenoid.chrome.Profile." + uuid.NewString()[:6]

	u := startedService.Url
	cancel := func(port int64) {
		log.Printf("[%d] [STOPPING CHROMEDRIVER]", requestId)
		startedService.Cancel()
		log.Printf("[%d] [REMOVING] [%s]", requestId, userDirPath)
		session.Ports.ReleasePort(port)
		err := os.RemoveAll(userDirPath)
		if err != nil {
			fmt.Printf("[%d] [REMOVING_TMP_FILE_FAILED] [%s] [%v]", requestId, userDirPath, err)
		}
	}
	host := "localhost"
	...
	i := 1
	port := session.Ports.GetFreePort(browserPortOffset)
	for ; ; i++ {
		r.URL.Host, r.URL.Path = u.Host, path.Join(u.Path, r.URL.Path)
		startedService.HostPort.Devtools = fmt.Sprintf("127.0.0.1:%d", port)
		newBody := removeSelenoidOptions(body)
		newBody = enrichGoogleChromeOptions(newBody, userDirPath, strconv.FormatInt(port, 10))
...
func enrichGoogleChromeOptions(input []byte, userDir string, port string) []byte {
	...
	//			GoogLoggingPrefs struct {
	//				Browser     string `json:"browser"`
	//				Performance string `json:"performance"`
	//			} `json:"goog:loggingPrefs"`
	...
	const chromeOptions = "goog:chromeOptions"
		...
		if c, ok := raw.(map[string]interface{}); ok {
			if raw, ok := c["firstMatch"]; ok {
				if fm, ok := raw.([]interface{}); ok {
					for _, raw := range fm {
						if c, ok := raw.(map[string]interface{}); ok {
							if raw, ok := c[chromeOptions]; ok {
								if raw, ok := raw.(map[string]interface{}); ok {
									var i interface{} = "--user-data-dir=" + userDir
									var p interface{} = "--remote-debugging-port=" + port
									raw["args"] = append(raw["args"].([]interface{}), i)
									raw["args"] = append(raw["args"].([]interface{}), p)
								}
							}
						}
					}
...
func devTools(hostFn func(sess *session.Session) string, mux *http.ServeMux) func(http.ResponseWriter, *http.Request) {
...
func browser(w http.ResponseWriter, r *http.Request) {
	u, err := getBrowserWebSocketUrl(r.URL.Host)
	if err != nil {
...
func getBrowserWebSocketUrl(dtHost string) (*url.URL, error) {
	dt := devtool.New(fmt.Sprintf("http://%s", dtHost))
	ver, err := dt.Version(context.Background())
	if err != nil {
...
func page(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[request] [%s]", r.URL.Host)
	for _, v := range fragments {
		log.Printf("[fragments] [%s]", v)

	}

	u, err := getPageWebSocketUrl(targetId, r.URL.Host)
	if err != nil {
...
func getPageWebSocketUrl(targetId string, dtHost string) (*url.URL, error) {
